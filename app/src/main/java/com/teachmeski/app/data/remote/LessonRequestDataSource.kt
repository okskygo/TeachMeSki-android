...
class LessonRequestDataSource @Inject constructor(
    ...
    suspend fun getRecommendedInstructors(
        ...
        supabaseClient.postgrest
            ...
            .select(
                columns = Columns.raw(
                    "id, user_id, display_name, avatar_url, short_id, rating_avg, rating_count, line_user_id, has_verified_certificate"
                )
            ) {
                filter {
    ...
    suspend fun getChatRoomsForRequest(
        ...
        supabaseClient.postgrest
            ...
            .select(
                columns = Columns.raw(
                    "id, instructor_id, last_message_content, last_message_at, last_message_sender_id, last_read_at, instructor_profiles(id, user_id, display_name, avatar_url, short_id, rating_avg, rating_count, line_user_id, has_verified_certificate)"
                )
            ) {
                filter {
